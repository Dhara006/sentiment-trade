...
def startup():
    import threading
    threading.Thread(target=df.fetch_all, daemon=True).start()

# --- News ---
@app.get("/api/news/{ticker}")
def get_news(ticker: str, hours: int = 24):
    news = df.get_news_for_ticker(ticker, hours=hours)
    if news.empty:
        return {"ticker": ticker, "articles": []}
    return {
        "ticker": ticker,
        "count": len(news),
        "articles": news.to_dict(orient="records"),
    }

# --- Tweets ---
@app.get("/api/tweets/{ticker}")
def get_tweets(ticker: str, hours: int = 24):
    tweets = df.get_tweets_for_ticker(ticker, hours=hours)
    if tweets.empty:
        return {"ticker": ticker, "posts": []}
    posts = tweets.to_dict(orient="records")
    for p in posts:
        p["textblob_sentiment"] = round(tb.analyze(p.get("text", "")), 3)
        p["vader_sentiment"] = round(vader.analyze(p.get("text", "")), 3)
...
def get_signals():
    ...
    if not all_news.empty and "base_sentiment" in all_news.columns:
        ...
        signals = sg.compute_multi_ticker_signals(all_news, df)
        cp = {t: (df.get_price_data(t)["Close"].iloc[-1] if not df.get_price_data(t).empty else 100) for t in TICKERS}
        for _, s in signals.iterrows():
            pt.execute_signal(s.to_dict(), cp.get(s["ticker"], 100))
        pt.update_equity(cp)
...
def get_portfolio():
    cp = {t: (df.get_price_data(t)["Close"].iloc[-1] if not df.get_price_data(t).empty else 100) for t in TICKERS}
    perf = pt.get_performance_summary(cp)
    def df_to_rows(d):
    ...
    perf["equity_curve"] = df_to_rows(eq)
